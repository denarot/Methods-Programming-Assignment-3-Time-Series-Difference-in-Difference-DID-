...
regression yields $F = 0.21$ ($p = 0.89$), providing statistical
...
($p = 0.97$), statistically indistinguishable from zero.  This null
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    ...
    \multicolumn{5}{l}{%
      \small Observations: 64.\quad $R^{2} = 0.997$.\quad
...
approximately 3{,}991 workers per county per year} relative to the
counterfactual trend established by Pennsylvania benchmark counties
($p < 0.001$; 95\% CI: [3{,}378,\;4{,}604]).  Because the parallel
trends assumption is strongly supported --- both visually and through
a statistically insignificant pre-period F-test ($p = 0.89$) --- and
because the placebo test yields a near-zero coefficient ($-3.3$,
$p = 0.97$), this estimate can be interpreted as causal: the subsidy,
